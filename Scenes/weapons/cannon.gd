...
func launch_ball(bullet:Variant, direction:Variant, location:Variant) -> void:
	...
	printt(rad_to_deg(direction), position, spawned_bullet.position)
	...
	var distance = (GameManager.player.global_position - global_position).x
...
func get_cannonball_position(pivot_position: Vector2, cannon_rotation: float, distance_from_pivot: float = -10) -> Vector2:
	...
	printt(pivot_position, cannon_rotation, distance_from_pivot, x, y)
	return Vector2(x, y)
...
var launch_angle = 45  # Angle in degrees
...
func compute_initial_speed(distance: float, time: float) -> Vector2:
	
	var g = 9.8  # Gravity
	var v_x = distance / time  # Horizontal velocity
	...
	# Calculate the required initial velocity to hit the target
	var initial_velocity = sqrt((abs(distance) * gravity) / sin(2 * theta))
	
	print_debug(initial_velocity, gravity)

	# Convert the velocity into a 2D vector (velocity components)
	velocity_x = initial_velocity if distance >= 0 else -initial_velocity * cos(theta)
	velocity_y = -initial_velocity * sin(theta)  # Negative because up is negative Y
